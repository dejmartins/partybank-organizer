...
import {
  createSeries,
  updateSeries,
} from "@/services/series-services/series-service";
...
export default function Page() {
  ...
  const handleUpdateSeries = async () => {
    ...
    const url =
      Object.keys(selectedFile).length !== 0
        ? await uploadToCloudinary(selectedFile, "event_image")
        : selectedImage;
    if (url) {
      const payload = {
      ...
      updateSeries({ payload, id: loadedSeries.id }).subscribe({
        next: (res) => {
          if (res) {
            console.log(res);
  ...
  useEffect(() => {
    const { name, image_url, description } = loadedSeries;
    setSeriesName(name);
  ...
  return (
    <div className="flex flex-col min-h-[calc(100vh-170px)] border-[var(--pb-c-soft-grey)]">
      ...
      <Loader isOpen={isLoaderModalOpen} message="Updating your series" />
    </div>
  );
}
